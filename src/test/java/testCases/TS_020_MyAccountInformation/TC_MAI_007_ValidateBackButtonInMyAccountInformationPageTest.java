...
// Step 7.1: Refresh the page to ensure fields are empty
            driver.navigate().refresh();

// Step 8: Verify previously entered values are lost
            Assert.assertEquals(driver.findElement(By.xpath("//input[@id='input-firstname']")).getAttribute("value"), "",
                    "First Name field is not empty after back navigation.");
            Assert.assertEquals(driver.findElement(By.xpath("//input[@id='input-lastname']")).getAttribute("value"), "",
                    "Last Name field is not empty after back navigation.");
