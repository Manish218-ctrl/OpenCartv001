...
public class TC_SPO_008_ValidateSpecialOffersPageFromWishListNavigationTest extends BaseClass {
    ...
    public void validateAddingProductToWishList() {
        ...
        // Step 6: Verify if the product is successfully added to the Wish List
        String productName = "Product Name";  // Replace with the actual product name if known
        logger.info("Verifying that the product is added to the Wish List...");
        boolean isProductAdded = wishListPage.isProductInWishList(productName);
        Assert.assertTrue(isProductAdded, "Product was not added to the Wish List.");
