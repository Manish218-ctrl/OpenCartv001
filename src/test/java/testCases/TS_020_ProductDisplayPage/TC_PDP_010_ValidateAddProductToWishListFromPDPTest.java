...
public class TC_PDP_010_ValidateAddProductToWishListFromPDPTest extends BaseClass {
    ...
    public void addProductToWishListFromPDP() {
        ...
        try {
            ...
            //Click on product from search results (Simplified call to the POM)
            ProductDisplayPage pdp = new ProductDisplayPage(getDriver()); // Re-use the pdp variable
            pdp.clickProductFromSearchResults(productName);
            logger.info("Clicked on product link: " + productName);

            //Initialize PDP and Click Add to Wish List
            ...
            //Click wish list link in success message
            pdp.clickWishListLink();
            logger.info("Navigated to Wish List page via success message link.");

            //Validate product is displayed in Wish List
            boolean isProductInWishList = getDriver().getPageSource().contains(productName);
