...
public class FeaturedSectionPage extends BasePage {
    ...
    public FeaturedSectionPage(WebDriver driver) {
        ...
        wait = new org.openqa.selenium.support.ui.WebDriverWait(
                driver,
    ...
    @FindBy(
            xpath = "(//div[@id='content']//div[contains(@class,'product-layout')])[1]//button[@data-original-title=Compare this Product] | " +
    ...
    public void hoverOnCompareButton() {
        wait.until(
                ExpectedConditions.visibilityOf(
                        btnCompareFirstFeatured
                )
        );

        new Actions(driver)
                .moveToElement(btnCompareFirstFeatured)
                .perform();
    }
    ...
    public boolean isCompareTooltipDisplayed() {
        try {
            String tooltipText =
                    btnCompareFirstFeatured.getAttribute(
                            "data-original-title"
                    );

            return tooltipText != null &&
                    tooltipText.trim()
                            .equalsIgnoreCase("Compare this Product");
